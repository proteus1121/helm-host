...
public class HelmController {
    ...
    @Autowired
    public HelmController(S3Service s3Service, HelmAttributeParser helmAttributeParser, @Value("${bucket}") String bucket) {
    ...
    @ResponseBody
    public FlexConfig upload(@RequestParam("file") MultipartFile body) throws IOException {
        HelmAttributes attributes = helmAttributeParser.getAttributes(AssetKind.HELM_PACKAGE, body.getBytes());
